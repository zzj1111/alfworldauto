...
#
# 4000 leaves real room to be specific. ALFWorld prompts carry an observation and an admissible-
# action list rather than a source file, so the scaffold competes against far less text here
# than in the kernel domains; the binding constraint was always the cap, not the budget.
...
def validate_action(action, domain=None):
    ...
    for op in text_ops:
        ...
        tgt = op.get("target")
        if tgt != GENERAL and tgt not in cats:
            return False, f"text_op target '{tgt}' is not '{GENERAL}' or a known category"
        txt = op.get("text")
...
def touched_tasks(text_ops, domain=None):
...
def apply_text_ops(scaffold, text_ops):
    ...
    for op in text_ops:
        if op["target"] == GENERAL:
            ...
            nxt["skills"][op["target"]] = op["text"].strip()
    nxt["version"] = scaffold.get("version", 0) + 1
    return nxt
