...
def decide(state, fns, cfg, ckpt, sr, ev, cyc):
    ...
    #
    # A floor sits under the pre-check. Measured across this harness's own arms: every arm that
    # ran WITHOUT triage wrote scaffolds (alf_from150 4/4 accepted, alf_scaffold_kl0 11 accepted,
    # alf_scratch150 2/2, alf_scratch150_pcap 10 accepted), and the one arm that ran WITH it
    # declined 17 of 20 cycles and accepted nothing — as did the CUDA arm, 17 of 20, in a
    # completely different domain. Every individual decline was defensible; the failure is that
    # "not yet" has no cost until the run is over. `intervene_floor_cycles` bounds how long the
    # scaffold may stay empty: past it the measurement is taken regardless, and the Teacher is
    ...
    forced = bool(floor) and (not S.has_text(state["scaffold"])) and cyc > floor
...
def prime(state, fns, cfg):
    ...
    ckpt = f"/mnt/data1/zha00175/gigpo_helper_ckpts/{cfg['exp']}/global_step_{state['step']}"
