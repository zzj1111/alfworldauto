...
TRAIN_SH = f"{ROOT}/examples/gigpo_trainer/run_search_3b_skill.sh"
ASSEMBLE_LOG = "/mnt/data1/zha00175/searchR1_data/assemble.log"
OUT = "/mnt/data1/zha00175/searchR1_data/orch_skill"
RETR_LOG = f"{OUT}/retriever.log"
TRAIN_LOG = f"{OUT}/train.log"
ORCH_LOG = f"{OUT}/orch.log"
DEADLINE_FILE = f"{OUT}/deadline.txt"
CKPT = "/mnt/data1/zha00175/ckpts_search_3b_skill"
PORT = 8010
HOURS = float(os.environ.get("SEARCH_HOURS", "24"))
TRAIN_TAG = "gigpo_3b_searchr1_skill"           # unique marker for OUR training proc
...
def gpu_line():
    return sh("nvidia-smi -i 1,2 --query-gpu=index,memory.used --format=csv,noheader").replace("\n", " | ").strip()
...
def latest_em():
    try:
        ...
        runs = wandb.Api().runs("mhong-university-of-minnesota/verl_agent_search",
                                filters={"display_name": "gigpo_3b_searchr1_skill"}, order="-created_at")
        r = runs[0]
        ...
        return (f"avg7(官方口径success)={avg:.1f}  [baseline无skill=42.9] @ wandb-step {r.summary.get('_step')} | "
...
def status_text(deadline, note=""):
    ...
    ck = sh("ls -d /mnt/data1/zha00175/ckpts_search_3b_skill/global_step_* 2>/dev/null | grep -oE 'global_step_[0-9]+' | tr '\\n' ' '").strip()
    return (f"{note}\n"
            f"=== Search-R1 GiGPO + Teacher-Skill (Qwen2.5-3B, GPU1) ===\n"
            f"训练={tup} | 检索器={rup} | 剩余监督窗口={rem}h\n"
            f"进度: step {train_step()}  (共 662 步)\n"
            f"评测EM(7数据集): {latest_em()}\n"
            f"已存 checkpoints: {ck or '(none)'}\n"
            f"GPU1(训练)/GPU2(检索): {gpu_line()}\n"
            f"wandb: https://wandb.ai/mhong-university-of-minnesota/verl_agent_search/runs/wjkfvatt\n"
...
def main():
    ...
    notify.send_email("[search-R1-SKILL] supervisor online",
    ...
    if port_up() and retriever_responds():
        ...
        if not retriever_ready(timeout=2400):
            notify.send_email("[search-R1-SKILL] retriever FAILED to start", status_text(deadline, "retriever not ready in 40min"))
        ...
        try:
            ...
            if not training_alive():
                ...
                if len(crash_times) >= 4:
                    notify.send_email("[search-R1-SKILL] training crash-loop — NEEDS ATTENTION",
            ...
            if time.time() - last_email >= 1800:
                notify.send_email("[search-R1-SKILL] 30-min status", status_text(deadline))
    ...
    notify.send_email("[search-R1-SKILL] 24h window done", status_text(deadline, "deadline reached"))
...
if __name__ == "__main__":
    try:
        ...
        try:
            notify.send_email("[search-R1-SKILL] SUPERVISOR FATAL - needs attention", _tb[:1500])
        except Exception:
            pass
        raise
