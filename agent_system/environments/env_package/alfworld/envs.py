...
class AlfworldWorker:
    ...
    def reseed(self, seed):
        """Re-seed so the NEXT reset() replays the same games as a previous pass.

        TextWorld's batch env advances its gamefile iterator on every reset ("Resetting this
        environment means starting the next game in the pool"), so consecutive passes over the
        same manager play DISJOINT game sets. seed() rebuilds that iterator from a seeded
        shuffle, which is what makes a paired A/B possible: re-seed identically before each arm
        and every arm sees the same games in the same order.
        """
        self.env.seed(seed)
        return True
...
class AlfworldEnvs(gym.Env):
    ...
    def reseed(self, seed=None):
        """Rewind every worker's game order. Uses the SAME per-worker offset as __init__, so
        after this call the next reset() yields exactly the games the first pass saw."""
        base = self._base_seed if seed is None else seed
        ray.get([w.reseed.remote(base + (i // self.group_n))
