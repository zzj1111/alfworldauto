...
def swap_to_bare_prompt(batch):
    """Re-condition a rollout batch on the UNSCAFFOLDED prompt, in place.

    Rollout is untouched: the trajectories were generated under the scaffolded prompt and the
    rewards/advantages computed from them stand. This only changes what the log-probs are
    conditioned on, so the policy is optimised on the prompt it actually faces at evaluation
    instead of on the one it trained under.

    Safe because every prompt is LEFT-padded to the same `max_prompt_length`: replacing the
    prompt block cannot shift the response block. The response half of `attention_mask` is
    carried over untouched and `position_ids` is recomputed from the new mask.

    Returns the number of rows swapped (0 when the batch carries no bare prompt, e.g. a
    multimodal arm or a run where the env manager did not emit one).
    """
    if "bare_input_ids" not in batch.batch or "bare_attention_mask" not in batch.batch:
        return 0, 0
    prompts = batch.batch["prompts"]
    responses = batch.batch["responses"]
    plen, rlen = prompts.shape[-1], responses.shape[-1]
    bare = batch.batch["bare_input_ids"].to(prompts.device)
    bare_mask = batch.batch["bare_attention_mask"].to(prompts.device)
    if bare.shape[-1] != plen:
        raise ValueError(f"bare prompt length {bare.shape[-1]} != prompt length {plen}; "
                         "both must be left-padded to data.max_prompt_length")

    # Rows where the scaffold was NOT injected (p=0 categories) already have bare == prompt,
    # so swapping them is a no-op. Counting the rows that actually DIFFER is the diagnostic
    # that tells us the loss is really being re-conditioned rather than silently unchanged.
    n_changed = int((bare != prompts).any(dim=-1).sum().item())

    resp_mask = batch.batch["attention_mask"][:, -rlen:]
    batch.batch["prompts"] = bare
    batch.batch["input_ids"] = torch.cat([bare, responses], dim=-1)
    batch.batch["attention_mask"] = torch.cat([bare_mask, resp_mask], dim=-1)
    batch.batch["position_ids"] = compute_position_id_with_mask(batch.batch["attention_mask"])
    return int(bare.shape[0]), n_changed
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    # Optionally re-condition the loss on the bare (unscaffolded) prompt.
                    #   mode "both"       -> swap BEFORE old_log_prob, so numerator AND
                    #                        denominator are pi(y|x). At ppo_epochs=1 the ratio
                    #                        is identically 1 and the update is the plain
                    #                        policy gradient A * grad log pi(y|x).
                    #   mode "numerator"  -> swap AFTER old_log_prob, giving the off-context
                    #                        importance ratio pi_theta(y|x)/pi_old(y|g(x)),
                    #                        which discounts trajectories that only succeeded
                    #                        because the scaffold was there.
                    _bare_cfg = self.config.algorithm.get("bare_prompt_loss", None)
                    _bare_on = bool(_bare_cfg and _bare_cfg.get("enable", False))
                    _bare_mode = (_bare_cfg.get("mode", "both") if _bare_cfg else "both")
                    if _bare_on and _bare_mode == "both":
                        _ns, _nc = swap_to_bare_prompt(batch)
                        metrics["bare_loss/n_swapped"], metrics["bare_loss/n_changed"] = _ns, _nc
                    ...
                    # "numerator" mode swaps HERE: old_log_prob above kept the scaffolded
                    # condition, giving the off-context ratio pi_theta(y|x)/pi_old(y|g(x)).
                    # It must happen before the reference pass, or the KL term would compare
                    # pi_theta(.|x) against pi_ref(.|g(x)) — a meaningless penalty.
                    if _bare_on and _bare_mode == "numerator":
                        _ns, _nc = swap_to_bare_prompt(batch)
                        metrics["bare_loss/n_swapped"], metrics["bare_loss/n_changed"] = _ns, _nc
