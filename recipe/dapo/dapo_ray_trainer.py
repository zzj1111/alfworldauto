            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        # single-turn: each response is its own trajectory -> unique traj_uid per
                        # row (verl-agent compute_grpo_outcome_advantage requires traj_uid for GiGPO
                        # step dedup; non-agent math has no shared steps, so each row is unique).
                        if "traj_uid" not in batch.non_tensor_batch:
                            _n = len(batch.batch)
                            batch.non_tensor_batch["traj_uid"] = np.array(
                                [str(uuid.uuid4()) for _ in range(_n)], dtype=object)
                            # non-agent single-turn: episode metrics = per-response scalars
                            batch.non_tensor_batch["episode_rewards"] = batch.batch["token_level_rewards"].sum(-1).cpu().numpy()
                            batch.non_tensor_batch["episode_lengths"] = batch.batch["response_mask"].sum(-1).cpu().numpy()
                            batch.non_tensor_batch["tool_callings"] = np.zeros(_n, dtype=np.float32)
